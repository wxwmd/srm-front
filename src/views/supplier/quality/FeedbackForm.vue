<template>
  <div class="jz-supplier-container">
    ...
    <div class="jz-module-toolbar">
      <div>
        <el-input class="jz-input" size="small" clearable v-model="tableUtil.filter.problemOdd" placeholder="问题单号"/>
        <el-input class="jz-input" size="small" clearable v-model="tableUtil.filter.feedbackTheme" placeholder="反馈主题"/>
        <el-input class="jz-input" size="small" clearable v-model="tableUtil.filter.feedbackPeople" placeholder="反馈人"/>
    ...
    <div class="jz-module-table">
      <el-table highlight-current-row
        ...
        <el-table-column prop="problemOdd" label="问题单号"/>
        <el-table-column prop="feedbackTheme" label="反馈主题"/>
        <el-table-column prop="feedbackPeople" label="反馈人"/>
        <el-table-column prop="feedbackSubmitDate" label="反馈提示时间"/>
        <el-table-column prop="feedbackDepartment" label="反馈部门"/>
        <el-table-column prop="receiptType" label="单据类型"/>
        <el-table-column prop="finishedProductCode" label="成品代码"/>
    ...
    <div class="jz-form">
      <el-dialog :title="tableUtil.form.title" :close-on-click-modal="false" :visible.sync="tableUtil.form.visible"
        ...
        <el-form :model="tableUtil.module.model" ref="form" :rules='form.formRules' label-width="100px"
                 label-position="left">
          <el-form-item label="问题单号" prop="problemOdd">
            <el-input size="small" class="jz-input" onkeyup="value=value.replace(/[\u4E00-\u9FA5]/g,'')"
                      v-model="tableUtil.module.model.problemOdd"
                      :disabled="!tableUtil.table.isAdd"/>
          </el-form-item>
          <el-form-item label="反馈主题" prop="feedbackTheme">
            <el-input size="small" class="jz-input" v-model="tableUtil.module.model.feedbackTheme"/>
          </el-form-item>
          <el-form-item label="反馈人" prop="feedbackPeople">
            <el-input size="small" class="jz-input" v-model="tableUtil.module.model.feedbackPeople"/>
          </el-form-item>
          <el-form-item label="反馈部门" prop="feedbackDepartment">
            <el-input size="small" class="jz-input" v-model="tableUtil.module.model.feedbackDepartment"/>
          </el-form-item>
          <el-form-item label="单据类型" prop="receiptType">
            <el-input size="small" class="jz-input" v-model="tableUtil.module.model.receiptType"/>
          </el-form-item>
          <el-form-item label="成品代码" prop="finishedProductCode">
            <el-input size="small" type="textarea" class="jz-input"
                      v-model="tableUtil.module.model.finishedProductCode"/>
          </el-form-item>
...
export default {
  ...
  created() {
    this.tableUtil = new TableUtil(this.$api.supplier.quality.feedbackForm, {}, 'QM反馈单');
    this.form.uploadUrl = this.$api.supplier.quality.feedbackForm.upload
    this.getSupplier()
  },
  methods: {
    ...
    update() {
      ...
      if (this.tableUtil.table.currentRow !== null) {
        this.$api.supplier.quality.feedbackForm.getVisible(this.tableUtil.table.currentRow.id).then(res => {
          if (res.code === 200) {
    ...
    download(data) {
      ...
      this.$api.supplier.quality.feedbackForm.download("blob", params).then(res => {
