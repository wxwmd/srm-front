  ...
  <div class="jz-supplier-container">
    ...
    <div class="jz-module-toolbar">
      ...
      <div v-if="type !== 1" class="jz-toolbar-margin1">
        <!--        <el-button type="primary" plain v-if="type == 1" @click="update()">上传</el-button>-->
    ...
    <div class="jz-form">
      <el-dialog :title="tableUtil.form.title" :close-on-click-modal="false" :visible.sync="tableUtil.form.visible"
                 destroy-on-close
                 width="800px">
        <el-form :model="tableUtil.module.model" ref="form" :rules='form.formRules' label-width="120px"
                 label-position="left">
          <el-form-item label="常用资料名称" prop="name">
            <el-input size="small" class="jz-input" v-model="tableUtil.module.model.name"/>
          </el-form-item>
          <el-form-item label="常用资料描述" prop="description">
            <el-input size="small" class="jz-input" v-model="tableUtil.module.model.description"/>
          </el-form-item>
...
export default {
  ...
  created() {
    this.tableUtil = new TableUtil(this.$api.supplier.collaboration.frequently, {}, '常用资料管理');
    this.form.uploadUrl = this.$api.supplier.collaboration.frequently.upload
    this.getSupplier()
  },
  methods: {
    ...
    update() {
      ...
      if (this.tableUtil.table.currentRow !== null) {
        this.$api.supplier.collaboration.frequently.getVisible(this.tableUtil.table.currentRow.id).then(res => {
          if (res.code === 200) {
    ...
    download(data) {
      ...
      this.$api.supplier.collaboration.frequently.download("blob", params).then(res => {
