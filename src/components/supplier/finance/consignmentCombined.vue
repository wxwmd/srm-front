<template>
  <div class="jz-form">
    <el-dialog title="开票" :close-on-click-modal="false" :visible.sync="combinedTicket.isCombined" width="1200px" destroy-on-close>
      <el-form :model="form.model" ref="form" :rules='form.formRules' label-width="100px">
        ...
        <el-form-item class="jz-from-flex" label-width="0">
          <el-form-item label="不含税金额" prop="amount" style="width: 25%">
            <el-input class="jz-input" onkeyup="value=value.replace(/[^\d.]/g,'')" v-model="form.model.resultList.amount"/>
          </el-form-item>
          <el-form-item label="税率(%)" prop="taxRate" style="width: 25%">
            <el-input class="jz-input" onkeyup="value=value.replace(/[^\d.]/g,'')" @change="calculate()" v-model="form.model.resultList.taxRate"/>
          </el-form-item>
          <el-form-item label="税额" prop="taxAmount" style="width: 25%">
            <el-input class="jz-input" onkeyup="value=value.replace(/[^\d.]/g,'')" @change="calculate()" v-model="form.model.resultList.taxAmount"/>
          </el-form-item>
  ...
  methods: {
    ...
    save(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.form.model.resultList.taxRate === ''){
